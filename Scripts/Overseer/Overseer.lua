...
function RetainersWaitingToBeProcessed()
    if DoesObjectExist("Summoning Bell") and GetDistanceToObject("Summoning Bell") < 3.5 then
        ForceARSave()
    end
    UpdateOverseerDataFile()
    Overseer_char_data = LoadOverseerCharacterData(Overseer_current_character)
...
local function Main()
    ...
    while true do
        ...
        if IsPlayerAvailable() and GetCharacterName() then
            ...
            LogInfo("[Overseer] Logged into character " .. Overseer_current_character)
            ...
            while not ar_finished do
                ...
                if (RetainersWaitingToBeProcessed() or SubsWaitingToBeProcessed()) then
                    ARSetMultiModeEnabled(true)
                end
                if not RetainersWaitingToBeProcessed() and not SubsWaitingToBeProcessed() and not ARIsBusy() then
